(() => {
  ...
  const layoutFragmentDefaults = {
    certificate: {
      code: { x: 23, y: 8.6, w: 54, h: 3.2 },
      meta: { x: 11, y: 12.8, w: 78, h: 7.5 },
      stamp: { x: 46.75, y: 17, w: 6.5, h: 7.4 },
      title: { x: 11, y: 24, w: 78, h: 10 },
      body: { x: 24, y: 39, w: 52, h: 19 },
      name: { x: 25, y: 65.5, w: 50, h: 8.2 },
      closing: { x: 27, y: 78, w: 46, h: 6.8 },
      author: { x: 39, y: 83.4, w: 22, h: 9.5 },
    },
    report: {
      code: { x: 13, y: 4.4, w: 74, h: 3.2 },
      meta: { x: 9, y: 9.5, w: 82, h: 6.6 },
      title: { x: 13, y: 15.4, w: 74, h: 8.2 },
      quote: { x: 10, y: 25.6, w: 80, h: 7.5 },
      instruction: { x: 10, y: 34.5, w: 80, h: 7 },
      lines: { x: 9, y: 44, w: 82, h: 33.5 },
      entry: { x: 10, y: 45.5, w: 80, h: 31 },
      signature: { x: 45, y: 86.5, w: 46, h: 7.5 },
      stamp: { x: 8, y: 79.5, w: 18, h: 12 },
    },
  };
    ...
    ["certificate", "report"].forEach((type) => {
      Object.keys(next[type]).forEach((id) => {
        ...
        ["x", "y", "w", "h"].forEach((key) => {
          ...
          next[type][id][key] = Math.min(max, Math.max(min, Math.round(value * 10) / 10));
        });
        next[type][id].x = Math.min(98, Math.max(0, next[type][id].x));
        next[type][id].y = Math.min(98, Math.max(0, next[type][id].y));
        next[type][id].w = Math.min(100 - next[type][id].x, Math.max(2, next[type][id].w));
        next[type][id].h = Math.min(100 - next[type][id].y, Math.max(2, next[type][id].h));
      });
    });
  ...
  function applyFragmentBox(element, fragment) {
    ...
    if (element.matches("img, .document-stamp, .document-author-signature, .report-lines, .report-entry")) {
  ...
  function applyFragmentLayout(sheet, type) {
    if (!sheet) return;
    $$("[data-layout-fragment]", sheet).forEach((element) => {
  ...
  function pdfFragmentRect(type, id, width, height) {
    const rect = fragmentRect(type, id, width, height);
    return {
      x: rect.x,
  ...
  function ensureLayoutSelection() {
    const labels = fragmentLabels[activeType];
    if (!labels[selectedLayoutFragment.id] || selectedLayoutFragment.type !== activeType) {
  ...
  function markSelectedLayoutFragment() {
    ...
    $(`[data-layout-fragment="${selection.id}"]`, sheet)?.classList.add("is-layout-selected");
  }
  ...
  function syncLayoutControls() {
    ...
    if (target) {
      const options = Object.entries(fragmentLabels[activeType])
        .map(([id, label]) => `<option value="${id}">${label}</option>`)
        .join("");
      if (target.dataset.optionsFor !== activeType) {
  ...
  function setSelectedLayoutFragment(id) {
    if (!fragmentLabels[activeType]?.[id]) return;
  ...
  function drawTransparentReportTitle(ctx, text, rect, scale = 1) {
    ...
    ctx.shadowOffsetY = -1;
    ctx.shadowBlur = 0;
    drawTypewriterInk(ctx, text, rect.x + rect.width / 2, rect.y + rect.height * .62);
    ctx.restore();
  }
  ...
  async function renderCertificate({ name, documentNumber = issueNumber(), width = A4.landscape.width } = {}) {
    ...
    drawSeal25D(ctx, stamp, stampRect.x, stampRect.y, stampRect.width, stampRect.height, .92, true);
    drawFinishedTitle(ctx, C().certificateTitle.toUpperCase(), titleRect.x + titleRect.width / 2, titleRect.y + titleRect.height * .72, {
    ...
    drawCenteredLines(ctx, wrapLines(ctx, C().certificateBody, bodyRect.width).slice(0, 9), bodyRect.x + bodyRect.width / 2, bodyRect.y + bodySize * 1.35, height * .03 * bodyScale);
    ...
    const nameY = nameRect.y + nameRect.height * .56;
    ctx.fillText(participantName, nameRect.x + nameRect.width / 2, nameY);
    const nameWidth = Math.min(ctx.measureText(participantName).width + nameSize * 1.2, nameRect.width);
    drawPremiumRule(ctx, nameRect.x + nameRect.width / 2 - nameWidth / 2, nameRect.x + nameRect.width / 2 + nameWidth / 2, nameY + nameSize * .22, width);
    ctx.fillStyle = "rgba(245,230,196,.72)";
    ctx.font = `${Math.round(width * .012 * bodyScale)}px DocSerif, serif`;
    drawCenteredLines(ctx, wrapLines(ctx, C().certificateClosing, closingRect.width).slice(0, 3), closingRect.x + closingRect.width / 2, closingRect.y + Math.round(width * .014), height * .024 * bodyScale);
    drawContained(ctx, author, authorRect.x, authorRect.y, authorRect.width, authorRect.height, .96);
    return canvas;
  }
  ...
  async function renderReport({ text, name, anonymous = false, documentNumber = issueNumber(), width = A4.portrait.width } = {}) {
    ...
    drawTransparentReportTitle(ctx, C().reportTitle.toUpperCase(), titleRect, titleScale);
    if (anonymous) {
      ctx.textAlign = "center";
    ...
    ctx.textAlign = "center";
    drawCenteredLines(ctx, wrapLines(ctx, quoteCopy[config.language][config.reportQuote], quoteRect.width).slice(0, 3), quoteRect.x + quoteRect.width / 2, quoteRect.y + width * .022, height * .026);
    ctx.fillStyle = "#33291d";
    ctx.font = `${Math.round(width * .015)}px DocTypewriter, monospace`;
    drawCenteredLines(ctx, wrapLines(ctx, C().reportInstruction, instructionRect.width).slice(0, 4), instructionRect.x + instructionRect.width / 2, instructionRect.y + width * .018, height * .022);
    ...
    reportLines.forEach((line, index) => drawTypewriterInk(ctx, line, entryRect.x + width * .01, entryRect.y + lineGap * .7 + index * lineGap));
    ...
    const witnessY = signatureRect.y + signatureRect.height * .42;
    ctx.fillText(witnessName, signatureRect.x + signatureRect.width, witnessY);
    ...
    ctx.beginPath();
    ctx.moveTo(signatureRect.x + signatureRect.width - witnessWidth, witnessY + witnessSize * .75);
    ctx.lineTo(signatureRect.x + signatureRect.width, witnessY + witnessSize * .75);
    ctx.stroke();
    ctx.font = `700 ${Math.round(width * .011)}px DocTypewriter, monospace`;
    drawTypewriterInk(ctx, C().witness.toUpperCase(), signatureRect.x + signatureRect.width, witnessY + witnessSize * 1.42);
    drawSeal25D(ctx, stamp, stampRect.x, stampRect.y, stampRect.width, stampRect.height, .74, false);
    ctx.textAlign = "center";
  ...
  function pdfTransparentReportTitle(page, text, font, rect, color, scale = 1) {
  ...
  async function createVectorPdf(type, data) {
    ...
    if (type === "certificate") {
      ...
      page.drawRectangle({ x: 40, y: 38, width: width - 80, height: height - 76, borderColor: palette.gold, borderWidth: 1, opacity: .74 });
       ...
       pdfSeal25D(page, stamp, stampRect.x, stampRect.y, stampRect.width, stampRect.height, .92, rgb(.01, .008, .004));
       pdfFinishedTitle(page, C().certificateTitle.toUpperCase(), cinzel, Math.min(43, titleRect.height * .56) * titleScale, titleRect.y + titleRect.height * .32, {
       ...
       bodyLines.forEach((line, index) => page.drawText(line, { x: bodyRect.x + (bodyRect.width - serif.widthOfTextAtSize(line, bodySize)) / 2, y: bodyRect.y + bodyRect.height - (index + 1) * 16.5 * bodyScale, size: bodySize, font: serif, color: palette.ivory }));
       ...
       const nameX = nameRect.x + (nameRect.width - nameTextWidth) / 2;
       const nameY = nameRect.y + nameRect.height * .42;
       ...
       pdfPremiumRule(page, nameRect.x + nameRect.width / 2 - nameWidth / 2, nameRect.x + nameRect.width / 2 + nameWidth / 2, nameY - 12, palette);
       ...
       closingLines.forEach((line, index) => page.drawText(line, { x: closingRect.x + (closingRect.width - serif.widthOfTextAtSize(line, closingSize)) / 2, y: closingRect.y + closingRect.height - (index + 1) * 12.5 * bodyScale, size: closingSize, font: serif, color: palette.ivory }));
       pdfContained(page, author, authorRect.x, authorRect.y, authorRect.width, authorRect.height, .96);
     } else {
      ...
      if (texture) page.drawImage(texture, { x: 0, y: 0, width, height, opacity: .19 });
       ...
       pdfTransparentReportTitle(page, C().reportTitle.toUpperCase(), typewriter, titleRect, palette.ink, titleScale);
       ...
       quoteLines.forEach((line, index) => page.drawText(line, { x: quoteRect.x + (quoteRect.width - typewriter.widthOfTextAtSize(line, 10.5)) / 2, y: quoteRect.y + quoteRect.height - (index + 1) * 14, size: 10.5, font: typewriter, color: palette.brown }));
       const instructionLines = pdfWrap(typewriter, C().reportInstruction, 8.5, instructionRect.width).slice(0, 4);
       instructionLines.forEach((line, index) => page.drawText(line, { x: instructionRect.x + (instructionRect.width - typewriter.widthOfTextAtSize(line, 8.5)) / 2, y: instructionRect.y + instructionRect.height - (index + 1) * 12.5, size: 8.5, font: typewriter, color: palette.ink }));
       ...
       for (let index = 0; index < 6; index += 1) {
         ...
         page.drawLine({ start: { x: linesRect.x, y }, end: { x: linesRect.x + linesRect.width, y }, thickness: .65, color: palette.brown, opacity: .55 });
         if (reportLines[index]) page.drawText(reportLines[index], { x: entryRect.x + 5, y: entryRect.y + entryRect.height - (index + 1) * lineGap + lineGap * .24, size: reportSize, font: typewriter, color: palette.ink });
       }
       ...
       const witnessY = signatureRect.y + signatureRect.height * .5;
       pdfRight(page, witnessName, nameFont, witnessSize, signatureRect.x + signatureRect.width, witnessY, palette.ink);
       const witnessWidth = Math.min(nameFont.widthOfTextAtSize(witnessName, witnessSize) + witnessSize, signatureRect.width);
       page.drawLine({ start: { x: signatureRect.x + signatureRect.width - witnessWidth, y: witnessY - 13 }, end: { x: signatureRect.x + signatureRect.width, y: witnessY - 13 }, thickness: .9, color: palette.ink, opacity: .85 });
       pdfRight(page, C().witness.toUpperCase(), typewriter, 6.8, signatureRect.x + signatureRect.width, witnessY - 34, palette.ink);
       pdfSeal25D(page, stamp, stampRect.x, stampRect.y, stampRect.width, stampRect.height, .74, rgb(.24, .16, .08));
